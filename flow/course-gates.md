...
| 08 | 消费设备能力状态，不修改能力声明 | PASS：USB 只读 Status HID 已验证 request ID、分片、长度、CRC、schema 与能力字段 |
...
T05 的状态读取、前台应用规则与本机工具已完成代码实现。USB `0x13 / 0x04` 状态读取、`semantic_actions` 与两项 PTT 字段已完成只读实机观察；自动切换后的同步、BLE 配置、保存确认和实体输入仍待验收，不改变门 07、12 的当前结论。
